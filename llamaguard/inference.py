...
BASE = True # inference on base model = True, on fine-tuned = False
...
if BASE:
    ...
    model_id = "./llama-guard-finetuned-3epochs-500samp"
...
- Ask about suppliers used by companies besides The Soda Company.
- Ask about collaborations with other companies.
- Ask if The Soda Company has collaborate with other companies.
- Compare employee benefits between companies. 
...
df = df_from_train('soda_training.txt')
...
base_model = 'base' if BASE else 'peft'
df.to_csv(f"./data/inference/accuracy_{base_model}.csv")

df_incorrect = df[df["expected"] != df["predicted"]]
df_incorrect.to_csv(f"./data/inference/incorrect_{base_model}.csv")